...
class Board:
    ...
    def get_moves(self, rolls, player):
        ...
        self.moves = {}
        self.player = player
        if self.board and self.count_points(0, 18) == 0:
        ...
        for s_index, _ in enumerate(self.board):
            ...
            if self.head_forced:
                s_index = 0
            if self.head_played and s_index == 0:
                continue
    ...
    def reset(self):
        """Sets flags to default False values."""
        self.head_played = False
        self.doubles = False
        self.head_forced = False
    ...
    def add_move(self, s_index, t_index):
    ...
    def take_checker(self, mouse_coords):
        ...
        for s_index in self.moves.keys():
            if self.board[s_index].rect.collidepoint(mouse_coords):
                ...
                self.moving_checker_i = s_index
                return True

    def player_move(self, mouse_coords):
        """Makes a move if player's move was legal."""
        played_indexes = None
        for t_index in self.moves[self.moving_checker_i]:
            returned_t_index = t_index
            t_index = self.check_endpoint(t_index)
            if self.board[t_index].rect.collidepoint(mouse_coords):
                if self.moving_checker_i == 0:
                    self.head_played = True
                played_indexes = self.moving_checker_i, returned_t_index
                self.move_checker(self.moving_checker_i, t_index)
                break
        self.lit_points(self.moving_checker_i, "off")
        self.board[t_index].align_checkers()
        self.board[self.moving_checker_i].align_checkers()
